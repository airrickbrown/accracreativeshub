...
export const signInWithGoogle = async (role: 'client' | 'designer' = 'client') => {
  // Save selected role before redirecting away
...
export const handleGoogleUser = async (user: any) => {
  ...
  // Use saved role from before OAuth redirect
  const savedRole = localStorage.getItem(OAUTH_ROLE_KEY) as 'client' | 'designer' | null
  const fallbackRole = user.user_metadata?.role || 'client'
  const role = savedRole || fallbackRole
  ...
  // Clear saved role once we've used it
