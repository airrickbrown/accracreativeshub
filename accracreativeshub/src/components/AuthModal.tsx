    ...
    <div style={{ position:'fixed', inset:0, zIndex:300, background:'rgba(0,0,0,0.95)',
      ...
      <div style={{ background:S.surface, border:`1px solid ${S.border}`, width:'100%', maxWidth:440, padding:40 }}>
        ...
        <div style={{ display:'flex', gap:1, background:S.borderFaint, marginBottom:32 }}>
          {(['login','signup'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ flex:1, background:tab===t ? S.goldDim : S.surface, border:'none',
                color:tab===t ? S.gold : S.textMuted, padding:'12px',
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {tab === 'signup' && (
            <>
              <Inp label="Full Name" placeholder="e.g. Kofi Mensah"
                value={form.fullName} onChange={v => f('fullName', v)} />
              <div>
                <Lbl style={{ marginBottom:8 }}>I am a</Lbl>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {[{ k:'client', l:'Client', sub:'I need design work' },
                    { k:'designer', l:'Designer', sub:'I offer design services' }].map(r => (
                    <div key={r.k} onClick={() => f('role', r.k)}
                      style={{ background:form.role===r.k ? S.goldDim : S.bgLow,
                        border:`1px solid ${form.role===r.k ? S.gold : S.border}`,
                        padding:'14px', cursor:'pointer', transition:'all 0.2s', textAlign:'center' }}>
          ...
          <Inp label="Email" placeholder="your@email.com"
            value={form.email} onChange={v => f('email', v)} />
          <Inp label="Password" placeholder="••••••••"
            value={form.password} onChange={v => f('password', v)} type="password" />

          {error && <Body style={{ color:S.danger, fontSize:12 }}>{error}</Body>}
          {success && <Body style={{ color:S.success, fontSize:12 }}>{success}</Body>}

          <Btn variant="gold" full
        ...
        <div style={{ marginTop:20, textAlign:'center' }}>
          <button onClick={onClose}
            style={{ background:'none', border:'none', color:S.textFaint,
