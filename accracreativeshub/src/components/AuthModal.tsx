...
//
// Props:
//   defaultTab?:  'login' | 'signup'      — which tab opens first
//   defaultRole?: 'client' | 'designer'   — pre-selects role (user can change)
//   lockRole?:    'client' | 'designer'   — hides role selector, forces this role
//
// KEY FIX: Before calling signInWithOAuth (Google), we write the selected role
// to localStorage so AuthCallback can read it after the redirect.
...
import { OAUTH_ROLE_KEY } from './AuthCallback'
...
// ── Scoped reset — kills all browser default button styles ──
const MODAL_STYLES = `
...
const FieldLabel = ({ text }: { text: string }) => (
  <p style={{ margin: '0 0 7px', fontFamily: S.headline, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: S.textFaint }}>
    {text}
  </p>
)
...
const Field = ({ label, type = 'text', placeholder, value, onChange, onEnter, suffix }: FieldProps) => {
...
export default function AuthModal({ onClose, defaultTab = 'login', defaultRole, lockRole }: AuthModalProps) {
  ...
  const [form, setForm]       = useState({
    ...
    // lockRole wins, then defaultRole, then 'client'
  ...
  // ── KEY FIX: store role BEFORE OAuth redirect so callback can read it ──
  const handleGoogle = async () => {
    setGLoading(true); setError('')

    // Write role to localStorage — survives the page redirect
    ...
    // On success: browser navigates away — no further code runs here
  }
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '10px 20px 0' }}>
          <button onClick={onClose} style={{ color: S.textFaint, fontSize: 22, lineHeight: 1, padding: 4 }} aria-label="Close">×</button>
        </div>
        ...
        <div style={{ padding: '4px 28px 0' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <p style={{ margin: 0, fontFamily: S.body, fontSize: 13, color: S.textMuted, lineHeight: 1.5 }}>
              {isLogin
                ...
                : lockRole === 'designer'
                ? 'Create your designer account to apply'
                : form.role === 'designer'
            ...
            {(['login', 'signup'] as const).map(t => (
              <button
                ...
                style={{
                  ...
                  fontFamily: S.headline, fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 700,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* Locked role badge — shown when lockRole is set */}
            {isSignup && lockRole && (
            ...
            <button
              ...
              disabled={gLoading}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', minHeight: 52, background: '#fff', border: 'none', borderRadius: 10, fontFamily: S.body, fontSize: 15, fontWeight: 600, color: '#1a1a1a', opacity: gLoading ? 0.65 : 1, transition: 'opacity 0.2s', boxShadow: '0 2px 12px rgba(0,0,0,0.25)' }}
            >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, paddingTop: 4 }}>
              ...
              {showResend && (
                <button
                  ...
                  disabled={loading}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', minHeight: 44, background: 'transparent', border: '1px solid rgba(201,168,76,0.22)', borderRadius: 9, fontFamily: S.body, fontSize: 13, color: S.gold }}
                >
