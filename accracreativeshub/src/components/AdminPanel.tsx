...
export default function AdminPanel({ onClose }: AdminPanelProps) {
  ...
  // Reject / remove from pending queue
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '100px 40px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 1, background: S.borderFaint, marginBottom: 32 }}>
          ...
          <StatCard
            ...
            value={`0${disputes.filter((d) => d.status === 'open').length}`}
        ...
        <div style={{ display: 'flex', gap: 1, background: S.borderFaint, marginBottom: 24 }}>
          {[
            ...
            <button
              ...
              style={{
                flex: 1,
                background: tab === t.k ? S.goldDim : S.surface,
                border: 'none',
                color: tab === t.k ? S.gold : S.textMuted,
                padding: '12px',
        ...
        {tab === 'applications' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {apps.length === 0 && (
              <Body style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            {apps.map((a) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Body style={{ fontSize: 11, marginBottom: 4 }}>
                    {a.category} · Applied {a.appliedAt}
                  </Body>
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    <span style={{ color: a.idVerified ? S.success : S.danger, fontSize: 10, fontFamily: S.body }}>
                      ● {a.idVerified ? 'ID Verified' : 'No ID uploaded'}
                    </span>
                    <span style={{ color: S.textFaint, fontSize: 10, fontFamily: S.body }}>
                      📁 {a.portfolioCount} samples
                    </span>
                    <span style={{ color: S.textFaint, fontSize: 10, fontFamily: S.body }}>
                      {a.phone}
                    </span>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn variant="outline" size="sm" onClick={() => alert('Opening portfolio review soon')}>
                    Review Portfolio
                  </Btn>
                  <Btn variant="danger" size="sm" onClick={() => rejectDesigner(a.id)}>
                    Reject
                  </Btn>
                  <Btn variant="success" size="sm" onClick={() => approveDesigner(a.id)}>
                    Approve
                  </Btn>
                </div>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {disputes.map((d) => (
              <div key={d.id} style={{ background: S.surface, border: '1px solid rgba(255,180,171,0.2)', padding: '20px 24px' }}>
                ...
                <Body style={{ fontSize: 11, marginBottom: 8 }}>
                  Client: {d.client} · Designer: {d.designer} · Raised: {d.raised}
                </Body>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <Btn variant="success" size="sm" onClick={() => { setDisputes((p) => p.filter((x) => x.id !== d.id)); alert('Funds released to designer.') }}>
                    Release to Designer
                  </Btn>
                  <Btn variant="danger" size="sm" onClick={() => { setDisputes((p) => p.filter((x) => x.id !== d.id)); alert('Refund issued to client.') }}>
                    Refund Client
                  </Btn>
                  <Btn variant="ghost" size="sm" onClick={() => alert('Evidence requested from both parties.')}>
                    Request Evidence
                  </Btn>
                </div>
              </div>
          ...
          <div>
            <div style={{ background: S.goldDim, border: `1px solid ${S.borderFaint}`, padding: '12px 16px', marginBottom: 16 }}>
              <Body style={{ fontSize: 11, color: S.textFaint }}>
                Featured slots appear at the top of the marketplace. Charging GH₵200/month per slot. 3 slots available.
              </Body>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {DESIGNERS.map((d) => (
                <div
                  ...
                  style={{
                    background: S.surface,
                    border: `1px solid ${d.featured ? S.gold : S.border}`,
                    padding: '14px 20px',
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <Btn
                      ...
                      size="sm"
                      onClick={() => alert(d.featured ? `${d.name} removed from featured` : `${d.name} added to featured — invoice GH₵200 sent`)}
                    >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              >
                <div>
                  ...
                  <Body style={{ fontSize: 11 }}>
                    {o.client} → {o.designer} {o.rush && <span style={{ color: '#fcd34d' }}>· ⚡ Rush</span>} · Revisions: {o.revisions.used}/{o.revisions.total}
                  </Body>
                </div>
                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                  ...
                  <div
                    style={{
                      background: o.status === 'delivered' ? 'rgba(74,154,74,0.12)' : S.goldDim,
                      color: o.status === 'delivered' ? S.success : S.gold,
                      fontSize: 9,
                      padding: '4px 10px',
                      letterSpacing: '0.15em',
                      textTransform: 'uppercase',
            ...
            >
              ...
              <Hl style={{ color: S.gold, fontSize: 20 }}>
                {fmt(ORDERS.reduce((s, o) => s + o.commission, 0))}
              </Hl>
            </div>
          </div>
        )}
