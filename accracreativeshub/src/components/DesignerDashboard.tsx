    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: S.bgDeep, overflowY: 'auto' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '88px 16px 60px' : '100px 40px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 280px', gap: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: '24px', flex: 1 }}>
              ...
              <Hl style={{ fontSize: 18, marginBottom: 10 }}><em style={{ fontStyle: 'italic', color: S.gold }}>Earn GH₵20 per referral</em></Hl>
              <Body style={{ fontSize: 11, marginBottom: 14 }}>Every new client you refer who completes their first order earns you GH₵20.</Body>
              <div style={{ background: S.bgLow, padding: '8px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                <code style={{ color: S.gold, fontSize: 10, fontFamily: 'monospace', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{`hub.gh/r/${designer.referralCode}`}</code>
                <button onClick={() => navigator.clipboard?.writeText(`hub.gh/r/${designer.referralCode}`).then(() => alert('Copied!'))} style={{ background: S.gold, color: S.onPrimary, border: 'none', padding: '4px 10px', fontSize: 9, fontFamily: S.body, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>COPY</button>
              </div>
        ...
        <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: isMobile ? '20px 16px' : '28px', marginBottom: 20, overflowX: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 1, background: S.borderFaint, minWidth: 480 }}>
            {['Project', 'Date', 'Status', 'Amount'].map(h => (
              <div key={h} style={{ background: S.surfaceHigh, padding: '8px 16px' }}><Lbl style={{ margin: 0, fontSize: 8 }}>{h}</Lbl></div>
            ))}
            {[
        ...
        <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: isMobile ? '20px 16px' : '28px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {Object.entries(BADGES).map(([key, b]) => {
              ...
              return (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: done ? S.goldDim : S.bgLow, border: `1px solid ${done ? S.gold : S.borderFaint}`, flexWrap: 'wrap' }}>
        ...
        <Btn variant="outline" full onClick={() => alert('Support contacted.')}>Need Technical Support? · Contact Support</Btn>
      </div>
    </div>
  )
}