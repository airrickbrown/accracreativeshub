...
export default function MessagingInterface({ onClose, initialOrder }: MessagingInterfaceProps) {
  ...
  const sendMessage = async (content: string, orderId: string | number, type = 'text', fileUrl?: string, fileName?: string) => {
  ...
  const approveAndReleaseFunds = async () => {
    ...
    setOrders((prev: any[]) => prev.map((o: any) => o.id === activeOrder.id ? { ...o, status: 'completed' } : o))
    setMsgs((m: any) => [...m, { from: 'system', text: 'Payment released. Order marked as completed.' }])
    setShowReview(true)
  }

  const submitReview = async (orderId: any, designerId: any, rating: number, comment: string) => {
    if (!user || !rating) { alert('Please select a rating'); return }
    setSubmittingReview(true)
    const { error: insertError } = await supabase.from('reviews').insert({ order_id: orderId, client_id: user.id, designer_id: designerId, rating, comment })
    if (insertError) { alert(insertError.message || 'Failed to submit review'); setSubmittingReview(false); return }
  ...
  const ConversationList = () => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {ordersLoading && <Body style={{ padding: '20px', fontSize: 12 }}>Loading...</Body>}
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0 }}>
      ...
      <div style={{ background: S.bg, padding: isMobile ? '12px 16px' : '14px 24px', borderBottom: `1px solid ${S.borderFaint}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          ...
          {isMobile && (
            <button onClick={() => setMobileView('list')} style={{ background: 'none', border: `1px solid ${S.borderFaint}`, color: S.textMuted, padding: '6px 10px', cursor: 'pointer', borderRadius: 6, fontFamily: S.headline, fontSize: 9, letterSpacing: '0.1em', flexShrink: 0 }}>
              ←
            </button>
          )}
          ...
          <div style={{ minWidth: 0 }}>
            <Hl style={{ fontSize: isMobile ? 13 : 15, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activeOrder.designer}</Hl>
      ...
      <div style={{ background: S.surface, padding: isMobile ? '10px 16px' : '12px 24px', borderBottom: `1px solid ${S.borderFaint}`, flexShrink: 0, overflowX: 'auto' }}>
        <div style={{ display: 'flex', gap: isMobile ? 16 : 24, alignItems: 'center', flexWrap: isMobile ? 'nowrap' : 'wrap', minWidth: isMobile ? 'max-content' : 'auto' }}>
          ...
          <div><Lbl style={{ marginBottom: 2, fontSize: 8 }}>Status</Lbl><Hl style={{ color: ord.status === 'delivered' || ord.status === 'completed' ? S.success : S.gold, fontSize: 11, textTransform: 'capitalize', whiteSpace: 'nowrap' }}>{String(ord.status).replace('_', ' ')}</Hl></div>
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            {view === 'client' && ord.status === 'in_progress' && (
              <Btn variant="outline" size="sm" disabled={revLeft === 0}
                onClick={() => {
            ...
            {view === 'client' && ord.status === 'delivered' && (
              <Btn variant="success" size="sm" onClick={approveAndReleaseFunds}>Approve & Release</Btn>
            )}
            {view === 'designer' && ord.status === 'in_progress' && (
              <Btn variant="gold" size="sm"
                onClick={() => {
                  setOrd((o: any) => ({ ...o, status: 'delivered' }))
                  setOrders((prev: any[]) => prev.map((o: any) => o.id === activeOrder.id ? { ...o, status: 'delivered' } : o))
      ...
      <div style={{ background: S.bg, padding: isMobile ? '10px 12px' : '14px 24px', borderTop: `1px solid ${S.borderFaint}`, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button
            ...
            disabled={uploading}
            style={{ background: 'none', border: `1px solid ${S.borderFaint}`, color: uploading ? S.textFaint : S.textMuted, width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: uploading ? 'not-allowed' : 'pointer', borderRadius: 6, flexShrink: 0, fontSize: 16, transition: 'all 0.2s' }}
          ...
          <input
            ...
            disabled={uploading}
            style={{ flex: 1, background: S.surface, border: `1px solid ${S.border}`, color: S.text, padding: '11px 14px', fontFamily: S.body, fontSize: 13, outline: 'none', borderRadius: 6, minHeight: 40 }}
          />
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
          {(view === 'client'
            ...
            <button key={r} onClick={() => setInput(r)} style={{ background: 'none', border: `1px solid ${S.borderFaint}`, color: S.textFaint, padding: '4px 10px', fontSize: 10, fontFamily: S.body, cursor: 'pointer', borderRadius: 4, whiteSpace: 'nowrap' }}>
              {r}
            </button>
          ))}
        </div>
