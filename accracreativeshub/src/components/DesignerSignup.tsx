...
const STEP_ERRORS: Record<number, (form: any) => string | null> = {
  ...
  2: (f) => {
    if (f.portfolioFiles.length < 1)
      return 'Please upload at least 1 portfolio sample to continue. (Minimum 5 required for full approval.)'
    return null
  },
  3: (f) => {
...
export default function DesignerSignup({ onClose }: DesignerSignupProps) {
  ...
  // ── Real portfolio file upload handler ──
  ...
  const removePortfolioItem = (index: number) => {
    const files    = form.portfolioFiles.filter((_, i) => i !== index)
    const previews = form.portfolioPreviews.filter((_, i) => i !== index)
    f('portfolioFiles', files)
    f('portfolioPreviews', previews)
  }

  // ── Real ID file upload handler ──
  const handleIdFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    f('idFile', file)
    f('idFileName', file.name)
    if (idInputRef.current) idInputRef.current.value = ''
  }

  // ── Real resume file upload handler ──
              ...
              <React.Fragment key={n}>
                <div
                  onClick={() => {
                    ...
                    if (n < step) { setStep(n); setStepError(null) }
                  }}
                  style={{ cursor: n < step ? 'pointer' : 'default', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}
                >
                  <div
                    style={{
                      width: 24, height: 24,
                      background: step === n ? S.gold : step > n ? 'rgba(201,168,76,0.4)' : S.surface,
                      border: `1px solid ${step === n ? S.gold : step > n ? S.gold : S.borderFaint}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      color: step === n ? S.onPrimary : step > n ? S.gold : S.textFaint,
                      fontSize: 9, fontFamily: S.body, fontWeight: 700,
                    }}
                  >
                    {step > n ? '✓' : n}
                  </div>
                ...
                {i < 3 && (
                  <div style={{ flex: 1, height: 1, background: step > n ? S.gold : S.borderFaint, margin: '0 6px 16px', opacity: step > n ? 0.4 : 1 }} />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <div>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{
                      ...
                      color: S.text, padding: '13px 44px 13px 16px', fontFamily: S.body, fontSize: 14,
                      outline: 'none', boxSizing: 'border-box', borderRadius: S.radiusSm, minHeight: 46,
                    }}
                  />
                  <button
                    onClick={() => f('showPassword', !form.showPassword)}
                    style={{
                      position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)',
                      background: 'none', border: 'none', color: S.textFaint, cursor: 'pointer', fontSize: 11,
                      fontFamily: S.body, padding: 0,
                    }}
                  >
              ...
              <Sel
                ...
                options={['', 'Logo Design', 'Flyer & Social Media', 'Business Branding', 'UI/UX Design', 'Motion Graphics', 'Photography']}
                value={form.category}
          ...
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <Body style={{ fontSize: 12 }}>
                Upload a minimum of 5 samples. Accepted formats: PNG, JPG, PDF.
                These will be reviewed by our editorial board.
              </Body>
              ...
              {form.portfolioPreviews.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                  {form.portfolioPreviews.map((preview, i) => (
                    <div
                      key={i}
                      style={{ position: 'relative', aspectRatio: '3/4', background: S.surface, border: `1px solid ${S.border}`, overflow: 'hidden' }}
                    >
                      <img
                        src={preview}
                        alt={`Sample ${i + 1}`}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(40%)', opacity: 0.85 }}
                      />
                      {/* Remove button */}
                      <button
                      ...
                      <div style={{ position: 'absolute', bottom: 4, left: 0, right: 0, textAlign: 'center' }}>
                        <Lbl style={{ margin: 0, fontSize: 6, background: 'rgba(0,0,0,0.5)', padding: '2px 4px' }}>
                          {form.portfolioFiles[i]?.name?.slice(0, 16)}
                        </Lbl>
                      </div>
                    </div>
                  ))}
              ...
              {form.portfolioPreviews.length === 0 && (
                <div
                  onClick={() => portfolioInputRef.current?.click()}
                  style={{
                    background: S.surface, border: `2px dashed ${S.border}`, borderRadius: S.radiusSm,
              ...
              <div
                style={{
                  background: form.portfolioFiles.length >= 5 ? 'rgba(74,154,74,0.08)' : 'rgba(201,168,76,0.06)',
                  border: `1px solid ${form.portfolioFiles.length >= 5 ? 'rgba(74,154,74,0.3)' : 'rgba(201,168,76,0.2)'}`,
                  padding: '12px 16px', borderRadius: S.radiusSm,
                }}
              >
                <Body style={{ fontSize: 12, margin: 0, color: form.portfolioFiles.length >= 5 ? S.success : S.gold }}>
                  {form.portfolioFiles.length === 0
                    ? 'No files uploaded yet — minimum 5 required for full approval.'
                    : form.portfolioFiles.length < 5
                    ? `${form.portfolioFiles.length} uploaded · ${5 - form.portfolioFiles.length} more needed for full approval`
                    : `✓ ${form.portfolioFiles.length} samples uploaded — you meet the minimum requirement`}
                </Body>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <Body style={{ fontSize: 12 }}>
                Government-issued identification is required to protect clients and maintain
                platform integrity. Your ID is encrypted and never shown publicly.
              </Body>

              {/* ID Upload */}
              <div>
                <Lbl style={{ marginBottom: 10 }}>Government-Issued ID * <span style={{ color: S.textFaint, fontSize: 9 }}>(Ghana Card or Passport)</span></Lbl>
                <div
                  style={{
                    background: S.surface,
                    border: `1px solid ${form.idFile ? 'rgba(74,154,74,0.4)' : S.border}`,
                    padding: '16px',
                ...
                >
                  <div>
                    ...
                    {form.idFileName && (
                      <Body style={{ fontSize: 11, margin: 0, color: S.textMuted }}>
                        {form.idFileName}
                      </Body>
                    )}
                  </div>
                  <Btn
                    variant={form.idFile ? 'outline' : 'gold'}
                    size="sm"
                    onClick={() => idInputRef.current?.click()}
                  >
                    {form.idFile ? 'Replace File' : 'Upload ID'}
                  </Btn>
                </div>
              </div>
              ...
              <div>
                <Lbl style={{ marginBottom: 10 }}>CV / Resume <span style={{ color: S.textFaint, fontSize: 9 }}>(Optional — PDF, DOC)</span></Lbl>
                <div
                  style={{
                    background: S.surface,
                    border: `1px solid ${form.resumeFile ? 'rgba(74,154,74,0.4)' : S.borderFaint}`,
                    padding: '16px',
                    borderRadius: S.radiusSm,
                    display: 'flex',
                    justifyContent: 'space-between',
                ...
                >
                  <div>
                    <Hl style={{ fontSize: 13, fontWeight: 500, marginBottom: 2 }}>
                      {form.resumeFile ? '✓ Resume Uploaded' : 'No file selected'}
                    </Hl>
                    {form.resumeFileName && (
                      <Body style={{ fontSize: 11, margin: 0, color: S.textMuted }}>{form.resumeFileName}</Body>
                    )}
                    {!form.resumeFile && (
                      <Body style={{ fontSize: 11, margin: 0 }}>Upload your CV to appear on your public profile</Body>
                    )}
                  </div>
                  <Btn
                    variant="ghost"
                  ...
                  >
                    {form.resumeFile ? 'Replace' : 'Upload Resume'}
                  </Btn>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: '20px', borderRadius: S.radiusSm }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 10 }}>
                  <div
                    style={{
                      width: 44, height: 44, borderRadius: '50%', background: '#1a2a1a',
                      border: `2px solid ${S.gold}`, display: 'flex', alignItems: 'center',
                      justifyContent: 'center', color: S.text, fontSize: 12, fontWeight: 700, fontFamily: S.body,
                    }}
                  >
                    {initials}
                  </div>
                ...
                {form.tagline && (
                  <Body style={{ fontSize: 12, lineHeight: 1.6, fontStyle: 'italic', borderTop: `1px solid ${S.borderFaint}`, paddingTop: 10, marginTop: 4 }}>
                    "{form.tagline}"
                  </Body>
                )}
                {form.bio && (
                  <Body style={{ fontSize: 11, lineHeight: 1.7, marginTop: 8, color: S.textMuted }}>
                ...
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {form.resumeFile && (
                    <span style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', color: S.gold, fontSize: 9, padding: '3px 8px', fontFamily: S.body, letterSpacing: '0.1em', textTransform: 'uppercase', borderRadius: 4 }}>
                      Resume Attached
                    </span>
                  )}
                  {form.price && (
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
            {step > 1 && (
              <Btn variant="ghost" onClick={() => { setStep((s) => s - 1); setStepError(null) }} full>
                ← Back
              </Btn>
            )}
            {step < 4 ? (
              // ── FIX: calls handleContinue which validates before proceeding ──
              <Btn variant="gold" onClick={handleContinue} full>
                Continue →
              </Btn>
            ) : (
