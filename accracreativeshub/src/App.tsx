...
import { S, kenteUrl, fmt } from './styles/tokens'
...
// When you add more designers or orders these update automatically
const REAL_STATS = {
  ...
  return (
    <div style={{ background:S.bg, minHeight:'100vh', color:S.text, fontFamily:S.body, backgroundImage:kenteUrl }}>
      ...
      <section id="hero" style={{ minHeight:'100vh', display:'flex', alignItems:'center', paddingTop:80, position:'relative', overflow:'hidden' }}>
        <div className="hero-grid" style={{ maxWidth:1200, margin:'0 auto', padding:'80px 40px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'center', width:'100%', position:'relative', zIndex:2 }}>
          ...
          <div style={{ animation: heroIn ? 'fadeUp 0.8s ease forwards' : 'none', opacity: heroIn ? 1 : 0 }}>
            ...
            <Hl style={{ fontSize:'clamp(42px,7vw,80px)', fontWeight:300, marginBottom:20, lineHeight:1.0 }}>
          ...
          <div className="hero-portrait" style={{ position:'relative', animation: heroIn ? 'fadeUp 0.8s ease 0.2s forwards' : 'none', opacity: heroIn ? 1 : 0 }}>
            <div style={{ position:'relative', overflow:'hidden' }}>
              <img src={DESIGNERS[0].portrait} alt={DESIGNERS[0].name}
        ...
        {/* Scroll indicator */}
        <div onClick={() => scrollTo('marketplace')} style={{ position:'absolute', bottom:32, left:'50%', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center', gap:8, animation:'pulse 2.5s ease infinite', cursor:'pointer' }}>
      ...
      <section style={{ background:S.surface, padding:'32px 40px', borderTop:`1px solid ${S.borderFaint}`, borderBottom:`1px solid ${S.borderFaint}` }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:1, background:S.borderFaint }}>
          {[
        ...
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div className="search-row" style={{ display:'flex', gap:12, marginBottom:28, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flex:1, minWidth:260 }}>
              ...
              />
              <button onClick={() => {}} style={{ background:S.gold, color:S.onPrimary, border:'none', padding:'11px 20px', fontFamily:S.headline, fontSize:10, letterSpacing:'0.15em', cursor:'pointer', textTransform:'uppercase', fontWeight:700, whiteSpace:'nowrap' }}>
                Search
              </button>
            </div>
            <div style={{ display:'flex', gap:1, background:S.borderFaint, flexWrap:'wrap' }}>
              {cats.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  style={{ background: category===c ? S.gold : S.surface, color: category===c ? S.onPrimary : S.textMuted, border:'none', padding:'11px 18px', fontFamily:S.headline, fontSize:9, letterSpacing:'0.15em', cursor:'pointer', textTransform:'uppercase', transition:'all 0.2s', fontWeight:700, whiteSpace:'nowrap' }}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom:16 }}>
            <Body style={{ fontSize:12, color:S.textFaint }}>{filtered.length} designer{filtered.length !== 1 ? 's' : ''} found{category !== 'All' ? ` in ${category}` : ''}{search ? ` matching "${search}"` : ''}</Body>
          </div>
      ...
      <section id="how-it-works" style={{ background:S.surface, padding:'96px 40px' }}>
        ...
        <div className="process-grid" style={{ maxWidth:900, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:1, background:S.borderFaint }}>
          {[
            ...
            <div key={i} onClick={s.action || undefined} style={{ background:S.bgLow, padding:'48px 32px', position:'relative', overflow:'hidden', textAlign:'center', cursor: s.action ? 'pointer' : 'default', transition:'background 0.2s' }}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:60, alignItems:'center' }}>
            <div>
              ...
              <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
                <Btn variant="gold"    size="lg" onClick={() => setShowSignup(true)}>Apply to Join →</Btn>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div className="footer-grid" style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', gap:48, marginBottom:48 }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:20 }}>
                <div>
                  <div style={{ color:S.gold, fontSize:20, fontFamily:S.headline }}>{REAL_STATS.designerCount}</div>
                  <Lbl style={{ margin:0, fontSize:8 }}>Designers</Lbl>
                </div>
                <div>
                  <div style={{ color:S.gold, fontSize:20, fontFamily:S.headline }}>{REAL_STATS.totalOrders}</div>
                  <Lbl style={{ margin:0, fontSize:8 }}>Orders</Lbl>
                </div>
                <div>
                  <div style={{ color:S.gold, fontSize:20, fontFamily:S.headline }}>{REAL_STATS.avgRating}★</div>
          ...
          <div style={{ borderTop:`1px solid ${S.borderFaint}`, paddingTop:24, display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:12 }}>
            <Body style={{ fontSize:11, margin:0 }}>
              © {new Date().getFullYear()} Accra Creatives Hub · Sovereign Craft · {REAL_STATS.commission}% commission on completed orders
            </Body>
            <div style={{ display:'flex', gap:20 }}>
