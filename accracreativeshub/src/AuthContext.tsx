...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const processUser = useCallback(async (u: any) => {
    ...
    // For Google users, ensure profile exists and get their role
    let role: string
    if (isGoogle) {
      role = await handleGoogleUser(u) || 'client'
    } else {
      role = await fetchRole(u.id, u.user_metadata?.role || 'client')
    }
  ...
  useEffect(() => {
    // Initial load — processes any OAuth hash tokens automatically
